...
const { exec } = require('child_process'); // 导入 child_process 的 exec
...
router.post('/', (req, res) => {
  upload(req, res, (err) => {
    ...
    if (req.files['terrainFile'] && req.files['terrainFile'][0]) {
      ...
      const targetPath = path.join(path.dirname(tempPath), terrainFile.filename); // 使用 path.dirname 获取临时目录
