...
// ========================================================================= //
// MODIFICATION: Removed redundant field declarations. 
// 解释: 以下场的声明已被移除，因为它们现在由 #include "createFields.H" 统一处理。
//       - volVectorField Cxy
...
/*******************************calc_CUDMI_ADM_INIT()*************************************/
#pragma region
    // MODIFICATION: Removed SourceT declaration. It's now loaded from createFields.H.
    // Just resetting the field to zero before calculation.
    SourceT *= 0.0;
    ...
    for (i = 0; i < s; i++)
    {
        ...
        for (j = 0; j < rou3; j++)
        {
            ...
            if (fabs(roua) <= lt / 2.0 && fabs(roub) <= lt / 2.0)
            {
                nx[validrou] = roua;
                ny[validrou] = roub;
                nh[validrou] = rou2 * vege_times / scale; // roughness height calc
                validrou = validrou + 1;
...
#pragma region
    ...
    long int num1 = 0, num3;
    int num_ceng = ceil(Foam::log((h - h1) * (q1 - 1) / (h1 / ceng) / q1 + 1) / Foam::log(q1)) + ceng;

    // 修正：使用保持维度的赋值方式
    Cxy *= 0.0;     // 保持 Cxy 的原有维度
    RoughT *= 0.0;  // 保持 RoughT 的原有维度
    
    forAll(mesh.cells(), cellI)
    {
        num1++;
        if (num1 <= 50 && Pstream::myProcNo() == 0) // Print only from master process
        {
            Pout << "Grid Num: " << cellI << "  Coordinate:" << mesh.C()[cellI].x() << "  " << mesh.C()[cellI].y() << "  " << mesh.C()[cellI].z() << endl;
        }
    }
    ...
    Info << "DT_R = " << DT_R << endl;
    Info << "r12 = " << r12 << endl;
    Info << "r12 * scale = " << r12 * scale << endl;
    Info << "Cd = " << Cd << endl;
    Info << "lad_max = " << lad_max << endl;
    Info << "vege_times = " << vege_times << endl;
    Info << "hh = " << hh << endl;
    Info << "maxFaceZ = " << maxFaceZ << endl;
    Info << "numFace = " << numFace << endl;
    Info << "actual_nodes = " << actual_nodes << endl;
    Info << "null_m = " << null_m << endl;
    Info << "null_n = " << null_n << endl;

    long int hf_nonzero_count = 0;
    long int height_match_count = 0;
    long int radius_check_count = 0;
    long int face_cell_match_count = 0;
    double max_hf = 0.0, min_hf = 1e10;
    double max_h0 = 0.0, min_h0 = 1e10;
    
    for (i = 0; i < numFace; i++)
    {
        double face_radius_sq = Foam::pow(faceX[i], 2) + Foam::pow(faceY[i], 2);
        double radius_limit_sq = r12 * scale * r12 * scale;
        ...
        if (face_radius_sq < radius_limit_sq)
        {
            ...
            {
                Info << "Face " << i << " passes radius check: (" 
                     << faceX[i] << ", " << faceY[i] << ", " << faceZ[i] 
                     << "), radius=" << Foam::sqrt(face_radius_sq) 
                     << ", limit=" << r12 * scale << endl;
            }
            ...
            if (k1 >= 0 && k4 < actual_nodes)
            {
                ...
                // 修正：转换 long double 为 double
                if (radius_check_count <= 5)
                {
                    Info << "  aa=" << aa << ", bb=" << bb << ", k1=" << k1 
                         << ", n3[k1]=" << static_cast<double>(n3[k1]) << ", hf=" << hf << endl;
                }
            }
            else
            {
                ...
                if (radius_check_count <= 5)
                {
                    Info << "  Array bounds error: k1=" << k1 << ", k4=" << k4 
                         << ", actual_nodes=" << actual_nodes << endl;
                }
            }
        }
        else
        ...
        forAll(mesh.cells(), cellI)
        {
            ...
            if (fabs(mesh.C()[cellI].x() - faceX[i]) < 0.00001 && fabs(mesh.C()[cellI].y() - faceY[i]) < 0.00001)
            {
                ...
                h0 = mesh.C()[cellI].z() - (2.0 * faceZ[i] - hh * scale) * h / (2.0 * h - hh);
                ...
                if (h0 >= 0 && h0 <= hf && hf != 0.0)
                {
                    ...
                    double roughness_value = 1.0 / scale * Cd * lad_max * Foam::exp(-(h0 / hf - 0.5) * (h0 / hf - 0.5) * 10.0);
                    ...
                    // 调试输出前几个匹配的网格
                    if (height_match_count <= 10)
                    {
                        Info << "Match " << height_match_count << ": cellI=" << cellI 
                             << ", face=" << i << ", coords=(" << mesh.C()[cellI].x() 
                             << "," << mesh.C()[cellI].y() << "," << mesh.C()[cellI].z() 
                             << "), h0=" << h0 << ", hf=" << hf 
                             << ", Cxy=" << roughness_value << endl;
                    }
                }
                else
                {
                    // 调试不匹配的条件
                    if (face_cell_match_count <= 10 && hf > 0.0)
                    {
                        Info << "No height match: cellI=" << cellI 
                             << ", h0=" << h0 << ", hf=" << hf 
                             << ", conditions: h0>=0?" << (h0>=0) 
                             << ", h0<=hf?" << (h0<=hf) 
                             << ", hf!=0?" << (hf!=0.0) << endl;
                    }
                }
    ...
    // 全局统计信息
    radius_check_count = returnReduce(radius_check_count, sumOp<long int>());
    hf_nonzero_count = returnReduce(hf_nonzero_count, sumOp<long int>());
    height_match_count = returnReduce(height_match_count, sumOp<long int>());
    face_cell_match_count = returnReduce(face_cell_match_count, sumOp<long int>());
    max_hf = returnReduce(max_hf, maxOp<double>());
    min_hf = returnReduce(min_hf, minOp<double>());
    max_h0 = returnReduce(max_h0, maxOp<double>());
    min_h0 = returnReduce(min_h0, minOp<double>());
    
    Info << "=== 粗糙度计算统计 ===" << endl;
    Info << "通过半径检查的面数: " << radius_check_count << endl;
    Info << "非零植被高度的面数: " << hf_nonzero_count << endl;
    Info << "面-网格匹配数: " << face_cell_match_count << endl;
    Info << "高度条件匹配数: " << height_match_count << endl;
    Info << "植被高度范围: [" << min_hf << ", " << max_hf << "]" << endl;
    Info << "网格高度范围: [" << min_h0 << ", " << max_h0 << "]" << endl;
...
#pragma region Debug_Roughness_Statistics
    ...
    Info << "粗糙度覆盖率: " << (double)roughCells/totalCells*100 << "%" << endl;
    Info << "最大粗糙度系数: " << maxCxy << endl;
    Info << "平均粗糙度系数: " << avgCxy << endl;
    
    // 输出粗糙度分布文件
    file1 = fopen("Output/Debug_Roughness_Distribution.txt", "w");
    fprintf(file1, "CellID\tX\tY\tZ\tCxy\n");
    forAll(mesh.cells(), cellI)
    {
        if (Cxy[cellI].x() > 0.0)
        {
            fprintf(file1, "%d\t%f\t%f\t%f\t%f\n", 
                cellI, mesh.C()[cellI].x(), mesh.C()[cellI].y(), 
                mesh.C()[cellI].z(), Cxy[cellI].x());
        }
    }
    fclose(file1);
#pragma endregion
...
#pragma endregion
#pragma region Debug_Roughness_Force
    double totalRoughForceX = 0.0, totalRoughForceY = 0.0, totalRoughForceZ = 0.0;
    long int activeRoughCells = 0;
    double maxRoughForce = 0.0;
    
    forAll(mesh.cells(), cellI)
    {
        double roughForceMag = Foam::sqrt(
            RoughT[cellI].x() * RoughT[cellI].x() + 
            RoughT[cellI].y() * RoughT[cellI].y() + 
            RoughT[cellI].z() * RoughT[cellI].z()
        );
        
        if (roughForceMag > 1e-10)  // 阈值避免数值误差
        {
            activeRoughCells++;
            totalRoughForceX += RoughT[cellI].x();
            totalRoughForceY += RoughT[cellI].y(); 
            totalRoughForceZ += RoughT[cellI].z();
            
            if (roughForceMag > maxRoughForce) maxRoughForce = roughForceMag;
        }
    }
    
    // 全局归约
    activeRoughCells = returnReduce(activeRoughCells, sumOp<long int>());
    totalRoughForceX = returnReduce(totalRoughForceX, sumOp<double>());
    totalRoughForceY = returnReduce(totalRoughForceY, sumOp<double>());
    totalRoughForceZ = returnReduce(totalRoughForceZ, sumOp<double>());
    maxRoughForce = returnReduce(maxRoughForce, maxOp<double>());
    
    Info << "=== 时间步 " << runTime.timeName() << " 粗糙度阻力信息 ===" << endl;
    Info << "产生粗糙度阻力的网格数: " << activeRoughCells << endl;
    Info << "粗糙度总阻力 (X,Y,Z): (" << totalRoughForceX << ", " 
         << totalRoughForceY << ", " << totalRoughForceZ << ")" << endl;
    Info << "最大粗糙度阻力大小: " << maxRoughForce << endl;
    Info << "粗糙度阻力总大小: " << Foam::sqrt(totalRoughForceX*totalRoughForceX + 
         totalRoughForceY*totalRoughForceY + totalRoughForceZ*totalRoughForceZ) << endl;
#pragma endregion
...
#pragma endregion
#pragma region Debug_Force_Comparison
    double totalTurbineForce = 0.0, totalRoughForce = 0.0;
    
    forAll(mesh.cells(), cellI)
    {
        totalTurbineForce += Foam::sqrt(
            SourceT[cellI].x() * SourceT[cellI].x() + 
            SourceT[cellI].y() * SourceT[cellI].y() + 
            SourceT[cellI].z() * SourceT[cellI].z()
        );
        
        totalRoughForce += Foam::sqrt(
            RoughT[cellI].x() * RoughT[cellI].x() + 
            RoughT[cellI].y() * RoughT[cellI].y() + 
            RoughT[cellI].z() * RoughT[cellI].z()
        );
    ...
    totalTurbineForce = returnReduce(totalTurbineForce, sumOp<double>());
    totalRoughForce = returnReduce(totalRoughForce, sumOp<double>());
    
    Info << "阻力对比 - 风机: " << totalTurbineForce 
         << ", 粗糙度: " << totalRoughForce 
         << ", 比例: " << (totalTurbineForce > 0 ? totalRoughForce/totalTurbineForce : 0) << endl;
#pragma endregion
...
#pragma region Debug_Output_Fields
        // 输出调试场，因为它们在createFields.H中被设置为AUTO_WRITE，
        // 所以runTime.write()会自动处理它们。
        // 如果需要强制写入，可以取消下面的注释。
        // if (runTime.write())
        // {
        //     Info << "Writing debug fields..." << endl;
        //     Cxy.write();
        //     RoughT.write();
        //     U_mag.write();
        // }
