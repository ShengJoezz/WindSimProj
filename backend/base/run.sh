...
CURVE_SRC_DIR="../customCurves"      # 源目录：持久化的用户上传目录
CURVE_DST_DIR="./run/Input"          # 目标目录：临时的计算输入目录
if [ -d "${CURVE_SRC_DIR}" ]; then
  echo "[Info] 发现自定义曲线目录 (${CURVE_SRC_DIR})，开始复制到 (${CURVE_DST_DIR})..."
  mkdir -p "${CURVE_DST_DIR}"
  # 复制用户上传的文件到计算目录的Input文件夹
...
# Step 11: Run admFoam in parallel
emit_task_start "run_admfoam"
mpirun --allow-run-as-root -n ${num_cores} admFoam -parallel  # 使用变量 ${num_cores}
if [ $? -ne 0 ]; then
  echo "{\"action\":\"progress\", \"progress\": \"ERROR\", \"taskId\":\"run_admfoam\"}"
  exit 1
fi
emit_progress 95 "run_admfoam"
sleep 1
