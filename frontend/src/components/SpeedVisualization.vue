...
import { Refresh, Download, Loading, Picture, InfoFilled } from '@element-plus/icons-vue';
import * as echarts from 'echarts';
import { debounce } from 'lodash-es';
import { ElMessage, ElSlider, ElSelect, ElOption, ElCard, ElIcon, ElButtonGroup, ElTooltip, ElRow, ElCol, ElDescriptions, ElDescriptionsItem } from 'element-plus';
// Import the updated service function
import { getMetadata, getSliceData, getProfileData, getWakeData, findClosestIndex, clearClientCaseCache } from '@/services/visualizationService';
...
const chartLoading = ref({ speedField: false, speedFieldImage: false, profile: false, wake: false });
...
const renderWakeChart = () => {
  ...
  try {
    ...
    const option = {
      title: { left: 'center', textStyle: { fontSize: 14, fontWeight: 'normal', color: colorScheme.text } },
      tooltip: { trigger: 'axis', formatter: (params) => { const data = params[0]; if (!data || data.value === undefined) return ''; const distance = data.value[0]; const formattedDist = distance >= 0 ? `下游 ${distance.toFixed(2)} km` : `上游 ${Math.abs(distance).toFixed(2)} km`; return `${formattedDist}<br>风速: ${data.value[1].toFixed(2)} m/s`; }, axisPointer: { type: 'cross' } },
      grid: { top: '10%', right: '8%', bottom: '15%', left: '15%' },
