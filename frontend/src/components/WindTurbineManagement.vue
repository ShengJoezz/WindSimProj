...
<template>
    <div class="wind-performance">
      ...
      <div class="tab-container">
        <div class="tab-buttons">
          ...
          <button class="tab-button" :class="{ active: activeTab === 'comparison' }" @click="switchTab('comparison')">调整前后对比</button>
  ...
  console.log("初始风速数据:", combinedData.value.map(item => item.initSpeed));
  console.log("调整后风速数据:", combinedData.value.map(item => item.adjSpeed));
  ...
  function renderPerformanceChangeChart() {
    ...
    const layout = {
      title: { text: '各风机性能调整前后变化率', font: { size: 16 } },
      autosize: true,
      xaxis: { title: '风机编号' },
      yaxis: { title: '变化率 (%)' },
      barmode: 'group',
    ...
    Plotly.newPlot(performanceChangeChart.value, data, layout, { responsive: true });
  }
